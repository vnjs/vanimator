...
function GeometryHelper() {
    ...
    function updatePolygonBillboardGeometry(
        ...
        face_indexes.forEach((f) => {
            geom_faces.push( new THREE.Face3(
                vert_map[f.a],
                vert_map[f.b],
                vert_map[f.c]
            ));
        ...
        geom_faces.forEach((face) => {
            const face_uv = [];
            face_uv.push( uvFromVertex( vertices[ face.a ] ) );
            face_uv.push( uvFromVertex( vertices[ face.b ] ) );
            face_uv.push( uvFromVertex( vertices[ face.c ] ) );
    ...
    // Computes the minimum distance from the given point (x, y) to the line
    // segment represented by the (x1, y1) and (x2, y2)

    function minimumDistancePointToLine(x, y, x1, y1, x2, y2) {
