...
function GeometryHelper() {
    ...
    function updatePolygonBillboardGeometry(
        ...
        vertices.forEach((v, i) => {
            vert_map[v.uid] = i;
            geom_vertices.push( new THREE.Vector3(v.x, v.y, 0) );
        });
        ...
        // const existing_face_vertex_uvs = geometry.faceVertexUvs[0];
        // if ( existing_face_vertex_uvs === undefined ||
        //      existing_face_vertex_uvs.length !== geom_faces.length ) {

        const geom_vertex_uv = [];
        geometry.faceVertexUvs[0] = geom_vertex_uv;

        function uvFromVertex(vertex) {
            const uvx = (vertex.x - x) / tex_width;
            const uvy = (-vertex.y + y) / tex_height;
