...
function GeometryHelper() {
    ...
    // property to the geometry to keep track of internal buffers when
    // updating.

    // NOTE: When the CPU is doing vertex morphs, this is called very
    //   frequently!
    ...
    function updatePolygonBillboardGeometry(
        ...
        function insertFaceUv(i, uv_buffer, uvi) {
            const v = vertices[ i ];
        ...
        for (let i = 0; i < face_indexes_length; ++i) {
            ...
            if (ia !== undefined && ib !== undefined && ic !== undefined) {
                ...
                // HACKY: Point to first index if face indexes do not match
                //   up with existing vertexes,
                faces_buffer[ n++ ] = 0;
                faces_buffer[ n++ ] = 0;
                faces_buffer[ n++ ] = 0;
            }
        ...
        // Write new buffer attributes if necessary,
        if (buffer_update === true) {
            geometry.addAttribute('position',
                new THREE.BufferAttribute( vert_buffer, 3).setDynamic(true));
            geometry.addAttribute('uv',
                new THREE.BufferAttribute( uv_buffer, 2 ).setDynamic(true));
            geometry.setIndex(
                new THREE.BufferAttribute( faces_buffer, 1 ).setDynamic(true));
        }
