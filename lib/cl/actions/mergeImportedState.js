...
module.exports = function mergeImportedState(editor, imported_ss) {
    ...
    i_layers.forEach((layer_ob) => {
        target_layer_set.push({
            uid: layer_ob.get('uid'),
            name: layer_ob.get('name'),
            mesh_uid_for_layer: layer_ob.get('mesh_uid_for_layer'),
        });
    });
    ...
    target_layer_set.forEach((dl) => {
        ...
        const FIELDS_TO_COPY = [
            ...
            'visible',
            'raw_texture_power', 'raw_texture_type', '$raw_texture_pixels'
        ];

        const lob = {
            uid: the_uid,
            path: path,
            mesh_uid_for_layer: dl.mesh_uid_for_layer,
        };
        if (dl.mesh_uid_for_layer !== undefined) {
            lob.mesh_uid_for_layer = dl.mesh_uid_for_layer;
        }
