    ...
    function initializeTextureLayers(ss, renderer, composition_controller) {
        ...
        texture_layers.forEach((layer) => {
            ...
            if (layer_type === 'layer') {
                ...
                let power_width = layer.getFromObject('extra_details', 'raw_texture_power_width');
                let power_height = layer.getFromObject('extra_details', 'raw_texture_power_height');
                const tex_type = layer.get('raw_texture_type');
                const pixels = layer.get('$raw_texture_pixels').data;

                // True if the pixel data already has premultiplied alpha.
                // False if not.
                let premultiplied_alpha_data =
                        layer.getFromObject('extra_details', 'premultiplied_alpha');
                if (premultiplied_alpha_data === undefined) {
                    premultiplied_alpha_data = false;
                }

                let mono_color_value =
                        layer.getFromObject('extra_details', 'mono_color_value');
                if (mono_color_value === undefined) {
                    mono_color_value = 0;
                }

                if (power_width === undefined) {
                    power_width = power;
                    power_height = power;
                }
                ...
                const texture = new THREE.DataTexture(
                                    pixels, power_width, power_height,
...
//                texture.minFilter = THREE.NearestFilter;
                ...
                texture.premultiplyAlpha = !premultiplied_alpha_data;
                ...
                present_material.uniforms.color_add.value = new THREE.Color(mono_color_value);
                ...
                pose_material.uniforms.color_add.value = new THREE.Color(mono_color_value);
