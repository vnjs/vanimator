...
function ModelCompositor(m_three_cache) {
    ...
    function getLayerDrawMesh(layer_uid) {
        ...
        if (view_mode === 'edit') {
            mesh = m_three_cache.getLayerData(layer_uid).present_mesh;
        }
        else if (view_mode === 'pose') {
            mesh = m_three_cache.getLayerData(layer_uid).pose_mesh;
        }
        else {
    ...
    function getGroupDrawDetails(group_uid) {
        ...
        // Get the mesh for this layer from threejs_cache,
        if (view_mode === 'edit') {
            const group_data = m_three_cache.getLayerData(group_uid);
            mesh = group_data.present_mesh;
            material = group_data.present_material;
        }
        else if (view_mode === 'pose') {
            const group_data = m_three_cache.getLayerData(group_uid);
            mesh = group_data.pose_mesh;
            material = group_data.pose_material;
        }
        else {
            throw Error('Unknown view mode');
        }
    ...
    function loadFrom(ss) {
        ...
        const track = Object.create(null);
        track['Art'] = root;
