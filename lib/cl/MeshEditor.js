...
const { minimumDistancePointToLine } = require('./GeometryHelper.js')();
...
function MeshEditor() {
    ...
    function loadFrom(ss, mesh_uid) {
        curved_deformer_line.loadFrom(ss, mesh_uid);
    ...
    function saveTo(ss, mesh_uid) {
        curved_deformer_line.saveTo(ss, mesh_uid);
    ...
    const points_transformer = TransformPoints(moveSingleVertex);
...
//        const in_face_indexes = data.face_indexes;
    ...
    function LineSegVertDist(distance, csegi) {
        return { distance, csegi };
    }
    ...
    function calcVertexDeformsForCurve(curved_2d_line) {
        ...
        const curve_points = curved_2d_line.createCurvePoints(8);
        ...
        if (curve_points.length > 1) {
            ...
            let v1 = curve_points[0];
            for (let i = 1; i < curve_points.length; ++i) {
                const v2 = curve_points[i];
                ...
                for (let n = 0; n < mlen; ++n) {
                    const mesh_v = vertex_arr[n];
                    const vuid = mesh_v.uid;
                    const distance = minimumDistancePointToLine(
                                mesh_v.rx, mesh_v.ry, v1.x, v1.y, v2.x, v2.y);
                    const cwi = vweight_map[vuid];

                    const distance_arr = cwi.distances;
                    if (distance_arr.length < 4) {
                        distance_arr.push( LineSegVertDist(distance, i - 1) );
                    }
                    else {
                        let insert = false;
                        let greatest_i = -1;
            ...
            console.log(vweight_map);
        }



        console.log("PENDING: Build weighted deform for curve.");
    }

    function deformMeshToCurve(curved_2d_line) {
        console.log("PENDING: Interactive deform mesh to curve.");
    }
