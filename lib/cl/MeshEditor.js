...
//        const in_face_indexes = data.face_indexes;
    ...
    function calcVertexDeformsForCurve(curved_2d_line) {
        ...
        if (curve_deform_points.length > 1) {
            ...
            for (let i = 1; i < curve_deform_points.length; ++i) {
                ...
                for (let n = 0; n < mlen; ++n) {
                    ...
                    if (distance_arr.length < 4) {
                        distance_arr.push( LineSegVertDist(distance, i - 1, mesh_v) );
                    }
                    else {
                        let insert = false;
                        let greatest_i = -1;
            ...
            // function addToInfluence(d) {
            //     // Pick the smallest distance,
            //     const { distance, csegi, vertex } = d;
            //
        ...
        // console.log(curve_deform_points);
        // console.log(curve_deform_line_influences);
    ...
    function deformMeshToCurve(curved_2d_line) {
        ...
        for (let i = 0; i < len; ++i) {
            ...
            if (infl !== undefined) {
                ...
                for (let n = 0; n < infl.length; ++n) {
                    ...
                    addVertexTranslate(vuid, vertex, angle, tx, ty, orig_cdp_s.x, orig_cdp_s.y);
        ...
        for (let vuid in vertex_translates) {
            ...
            for (let i = 0; i < ob.length; ++i) {
                ...
                const temp_x = work_x;
                const temp_y = work_y;
                work_x = (Math.cos(angle) * (temp_x - origin_x)) - (Math.sin(angle) * (temp_y - origin_y)) + origin_x;
                work_y = (Math.sin(angle) * (temp_x - origin_x)) + (Math.cos(angle) * (temp_y - origin_y)) + origin_y;
                work_x += tx;
                work_y += ty;
            }
