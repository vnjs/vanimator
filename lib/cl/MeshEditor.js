...
function MeshEditor() {
    ...
    // Vertex translate,
    function translateWeightedVertexes(weighted_ctx, dx, dy) {
        const { weighted_vs } = weighted_ctx;
        for (let vuid in weighted_vs) {
            const v = weighted_vs[vuid];
    ...
    // Vertex rotate,
    function rotateWeightedVertexes(weighted_ctx, sx, sy, dx, dy) {
    ...
    // Vertex scale,
    function scaleWeightedVertexes(weighted_ctx, dx, dy, view_scale) {
        calcMidpoint(weighted_ctx);
        const { weighted_vs, mid } = weighted_ctx;
        let dst = -dy;
