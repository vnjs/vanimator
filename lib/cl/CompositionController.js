...
function CompositionController(threejs_cache) {
    ...
    function calcMeshMorphFromAnimKey(ss, ak_ob, value) {
        ...
        let findVertOp = findVert;
        let lower_verts = lower_mt.get('target_verts');
        let upper_verts = upper_mt.get('target_verts');
        if (lower_verts.length > 24) {
            findVertOp = findVertInMap;
            lower_verts = convertVertsToMap(lower_verts);
            upper_verts = convertVertsToMap(upper_verts);
        }

        const mesh_uid = lower_mt.get('target_mesh_uid');

        const rv = dif > 0.01 ? (value - lower_target) / dif : 0;
        ...
        for (let i = 0; i < len; ++i) {
            ...
            const lvchange = findVertOp(lower_verts, vert.uid);
            const uvchange = findVertOp(upper_verts, vert.uid);

            if (lvchange !== undefined && uvchange !== undefined) {

                const dx = (lvchange.dx * l_mult) + (uvchange.dx * u_mult);
                const dy = (lvchange.dy * l_mult) + (uvchange.dy * u_mult);
        ...
        anim_keys.forEach((anim_key) => {
            ...
            for (let mesh_uid in deform_details) {
                ...
                if (all_mdetails === undefined) {
                    ...
                    const len = all_mdetails.vertex_arr.length;
                    for (let i = 0; i < len; ++i) {
                        const v1 = all_mdetails.vertex_arr[i];
                        const v2 = findVert(details.vertex_arr, v1.uid);
                        if (v2 !== undefined) {
                            v1.dx += v2.dx;
                            v1.dy += v2.dy;
                        }
...
//                    all_mesh_deforms[mesh_uid] = all_mdetails;
    ...
    function mergeVertexDeforms(vertex_arr1, vertex_arr2) {

        const out_vertex_arr = [];

        const len = vertex_arr1.length;
        for (let i = 0; i < len; ++i) {
            const v1 = vertex_arr1[i];
            const v2 = findVert(vertex_arr2, v1.uid);
            if (v2 === undefined) {
                out_vertex_arr.push(v1);
            }
            else {
                out_vertex_arr.push({
                    uid: v1.uid,
    ...
    // function updatePoseFromSingleAction(ss, action_uid, value) {
    //
    //     const all_mesh_deforms =
    //                     calcMeshMorphsFromSingleAction(ss, action_uid, value);
    //
    //     // Interactively update pose geometry from the deform information,
    //     for (let mesh_uid in all_mesh_deforms) {
    //         const deform = all_mesh_deforms[mesh_uid];
    //
    //         deform.layers_set.forEach((layer_uid) => {
    //             updatePoseGeometry(layer_uid, deform.vertex_arr, deform.face_indexes);
    //         });
    //
    //     }
    //
