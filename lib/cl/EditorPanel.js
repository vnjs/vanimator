...
function EditorPanel(window, document) {
    ...
    const replay_history = ReplayHistory();
...
//        camera.needsUpdate = true;
    ...
    function getModelTreeModel() {
        ...
        const meshes_arr = ss.getArray('meshes');

        if (meshes_arr.getLength() > 0) {

            const out = [
                { type:'branch', path:[], name:'Meshes' }
            ];
            const len = meshes_arr.getLength();
            for (let i = 0; i < len; ++i) {
                const layer = meshes_arr.get(i);
    ...
    function refreshScene() {
        ...
        for (let i = 0; i < len; ++i) {
            ...
            if (layer.get('visible') === true) {
                ...
                if (visible) {
                    if (layer_type === 'group') {
                        const group_path_vis = layer_path.concat(layer.get('name'));
                        visible_paths.push(group_path_vis);
                    }
                    else if (layer_type === 'layer') {
                        const mesh = threejs_cache[layer_uid].mesh;
...
//        console.log(evt);
    ...
    function initializeLoadedState() {
        ...
        for (let i = len - 1; i >= 0; --i) {
            const layer = meshes_arr.get(i);
            const layer_type = layer.get('type');
            const layer_raw_texture_power = layer.get('raw_texture_power');
            const layer_$raw_texture_pixels = layer.get('$raw_texture_pixels');
            if (layer_type === 'layer') {

                const power = layer_raw_texture_power;
                const pixels = layer_$raw_texture_pixels.data;
