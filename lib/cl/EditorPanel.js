...
const ThreeCache = require('./ThreeCache.js');
...
//    let threejs_cache = {};
    ...
    // Displays some debugging information in the console,
    function printDebugToConsole() {

        console.log("Dumping debugging information:");

        const anim_morphs_per_mesh = {};
        const ss = getSerializedState();

        const morph_targets = ss.getArray('morph_targets');
        const anim_keys = ss.getArray('anim_keys');
        anim_keys.forEach((anim_key) => {
            const keyframe_data = anim_key.get('keyframe_data');

            const keyed_morph_sequence = [];
            let for_mesh_uid;

            keyframe_data.forEach((keyframe_d) => {
                const time_point = keyframe_d.time;
                const morph_target_uid = keyframe_d.morph_target_uid;
                if (morph_target_uid !== undefined) {
                    const morph_target = morph_targets.get(morph_target_uid);
                    const mesh_uid = morph_target.get('target_mesh_uid');
                    for_mesh_uid = mesh_uid;
                    const verts = morph_target.get('target_verts');
                    keyed_morph_sequence.push({ time_point, morph_target_uid, verts });
                }
            });

            if (for_mesh_uid !== undefined) {
                let sarr = anim_morphs_per_mesh[for_mesh_uid];
                if (sarr === undefined) {
                    sarr = [];
                    anim_morphs_per_mesh[for_mesh_uid] = sarr;
                }
                sarr.push(keyed_morph_sequence);
            }
        ...
        for (let mesh_uid in anim_morphs_per_mesh) {
            const sarr = anim_morphs_per_mesh[mesh_uid];
            const mesh_ob = meshes.get(mesh_uid);
            const layer_uids = mesh_ob.get('layers_set');
            console.log("Mesh of:");
            let layer_names = '';
            layer_uids.forEach((layer_uid) => {
                const layer_ob = layers.get(layer_uid);
                if (layer_names !== '') {
                    layer_names += ', ';
                }
                layer_names += layer_ob.get('name');
            });
            console.log("  " + layer_names);
            console.log("  (%s Morph Targets on this Mesh)", sarr.length);
            console.log(sarr);
        }
        ...
        // const morph_target_per_mesh = {};
        //
        // const ss = getSerializedState();
        // const morph_targets = ss.getArray('morph_targets');
        // morph_targets.forEach((morph_targets_ob) => {
        //     const target_mesh_uid = morph_targets_ob.get('target_mesh_uid');
        //     let mt_arr = morph_target_per_mesh[target_mesh_uid];
        //     if (mt_arr === undefined) {
        //         mt_arr = [];
        //         morph_target_per_mesh[target_mesh_uid] = mt_arr;
        //     }
        //     mt_arr.push({
        //         morph_target_uid: morph_targets_ob.get('uid'),
        //         verts: morph_targets_ob.get('target_verts')
        //     });
        // });
        //
        // const meshes = ss.getArray('meshes');
        // const layers = ss.getArray('texture_layers');
        //
        // for (let mesh_uid in morph_target_per_mesh) {
        //     const all_mesh_morph_targets = morph_target_per_mesh[mesh_uid];
        //     const mesh_ob = meshes.get(mesh_uid);
        //     const layer_uids = mesh_ob.get('layers_set');
        //     console.log("Mesh of:");
        //     let layer_names = '';
        ...
        //     });
        //     console.log("  " + layer_names);
        //     console.log("  (%s Morph Targets on this Mesh)", all_mesh_morph_targets.length);
        // }

//        console.log(morph_target_per_mesh);
...
//        camera.needsUpdate = true;
    ...
    function removeTopHistory() {
        replay_history.removeTop();
    }
    ...
    function isInCurveModeEdit() {
        const current_tool_mode =
                    ss.getFromObject('editor_scratch', 'current_tool_mode');
        const curve_edit_tool_active = (current_tool_mode === 'curve_mode');
        return ( mesh_editor.getMode() === 'edit' &&
                 curve_edit_tool_active === true );
    }

    function isInCurveModePose() {
        const current_tool_mode =
                    ss.getFromObject('editor_scratch', 'current_tool_mode');
        const curve_edit_tool_active = (current_tool_mode === 'curve_mode');
        return ( mesh_editor.getMode() === 'pose' &&
                 curve_edit_tool_active === true );
    }
...
//        }
    ...
    function startMeshEdit(r, edit_type) {
        const weighted_selection = createWeightedVertexSelection();
        if (weighted_selection.affect_count > 0) {
            active_mesh_editing = {
                edit_class: 'vertex',
                edit_type,
                weighted_selection: weighted_selection,
    ...
    function handleCompleteMeshEditing(evt) {
        if (current_selected_mesh_uid !== undefined) {
            ...
            if ( current_tool_mode === 'curve_mode' ) {
                // If it 'EDIT mode.'
                const mesh_editor_mode = mesh_editor.getMode();
                if ( mesh_editor_mode === 'edit' ) {
                    const curved_2d_line = mesh_editor.getCurvedDeformerLine();
                    mesh_editor.calcVertexDeformsForCurve(curved_2d_line);
                }
            }
            saveCurrentMesh();
        }
...
//        console.log(evt);
    ...
    function handleGeneralUndoRedoEvent(actionFn) {
        const prev_editor_scratch = ss.get('editor_scratch');
        const did_state_change = actionFn();
        if (!did_state_change) {
            ...
            // Did tool modes change?
            const cur_editor_scratch = ss.get('editor_scratch');

            if ( prev_editor_scratch.current_view_mode !==
                 cur_editor_scratch.current_view_mode ||
                 prev_editor_scratch.current_tool_mode !==
                 cur_editor_scratch.current_tool_mode
               ) {

                edit_mesh_mode_select_el.value =
                                        cur_editor_scratch.current_view_mode;
                setMeshMode(cur_editor_scratch.current_view_mode);
                toggleEditTool(cur_editor_scratch.current_tool_mode);
    ...
    function handleUndoEvent() {
        handleGeneralUndoRedoEvent(() => {
            saveCurrentMesh();
            return replay_history.undo();
        });
    }

    function handleRedoEvent() {
        handleGeneralUndoRedoEvent(() => {
            const redo_result = replay_history.redo();
            return redo_result;
        });
    }
    ...
    function startInteractiveTransform(edit_type) {
        if (current_selected_mesh_uid !== undefined) {
            ...
            if (current_tool_mode === 'mesh_edit_mode') {
                is_transforming = startMeshEdit(r, edit_type);
            }
            else if (current_tool_mode === 'curve_mode') {
                is_transforming = startCurveEdit(r, edit_type);
            }
            else {
                throw Error('Unknown tool mode: ' + current_tool_mode);
            }

            // Capture mouse events if we are transforming,
            if (is_transforming === true) {
    ...
    // Returns the current editor instance (either mesh or curve deformer),
    ...
    function mirrorSelectedVertexes(mirror_x, mirror_y) {
        console.log("MIRROR %s, %s", mirror_x, mirror_y);
        ...
        if (selected_uids.length > 0) {
            saveCurrentMesh();
            checkpointHistory();
            editor.mirrorVertexes(selected_uids, mirror_x, mirror_y);
            saveCurrentMesh();
        }
        update_mesh_editor = true;
    ...
    function selectedVertexesToClipboard(setClipboard) {
        ...
        const to_clip_str = JSON.stringify(selected_data, null, 2);
        setClipboard(to_clip_str);
    }


    function resetVertexesToRest() {
        const editor_scratch = ss.get('editor_scratch');
        if (editor_scratch.current_view_mode === 'pose') {
            const editor = getCurrentEditorInstance();
            const selected_uids = editor.getAllSelectedIndexes();
            if (selected_uids.length > 0) {
                saveCurrentMesh();
                checkpointHistory();
                editor.resetToRest(selected_uids);
                update_mesh_editor = true;
                updatePoseMesh( current_selected_mesh_uid,
                                mesh_editor.getCurrentMeshDetails() );
                saveCurrentMesh();
    ...
    function handleEditorPasteOperation(selected_data) {
        ...
        console.log("PASTE: ", selected_data);
        if (isInMeshModeEdit()) {
    ...
    function handleEditorKeyDown(evt) {
        ...
        // Don't consume keys if the modal dialog is showing,
        if (modal_dialog.isShowing()) {
            return;
        }
        ...
        if (current_selected_mesh_uid !== undefined) {
            ...
            if (kc === KeyCode.DELETE) {
                if (isInMeshModeEdit()) {
                    ...
                    // If there's vertexes being deleted,
                    if (selected_uids.length > 0) {
                        saveCurrentMesh();
                        checkpointHistory();
            ...
            // Fill line or face,
                ...
                if (isInMeshModeEdit()) {
                    ...
                    if (selected_uids.length === 2) {
                        ...
                        const primary_select_vertex =
                                    mesh_editor.getPrimarySelectVertex();
                        mesh_editor.selectNone();
                        mesh_editor.selectVertex(primary_select_vertex);
            ...
            // Reset selected uids to rest position (in edit mode),
            else if (c === 'p') {
                console.log("Reset vertexes to rest...");
                resetVertexesToRest();
            }
...
//        const selected_point_uid = active_mesh_editing.selected_point_uid;
        ...
        // Interactive deform mesh around curve,
        if (isInCurveModePose()) {
            mesh_editor.deformMeshToCurve(curved_2d_line);
    ...
    function setMeshMode(mode_change_value) {
        ss.setInObject('editor_scratch', 'current_view_mode', mode_change_value);
    ...
    function createLatticeFromCurve(lattice_resolution, lattice_width) {
        const createLatticeFromCurve = require('./actions/createLatticeFromCurve.js');
        createLatticeFromCurve(editor, lattice_resolution, lattice_width);
    }
    ...
    function doCreateLatticeAction() {
        modal_dialog.setForm({
            type: 'create_lattice_form',
            title: 'Create lattice from curved line:',
            accept: (form) => {
                const lattice_resolution = parseInt(form.fields.resolution, 10);
                const lattice_width = parseFloat(form.fields.width);
                createLatticeFromCurve(lattice_resolution, lattice_width);
            }
        });
        modal_dialog.show();
    }
    ...
    function toggleEditTool(mode) {
        ...
        ss.setInObject('editor_scratch', 'current_tool_mode', mode);
        last_edit_ob = t;
...
<select id="layer_blend_mode" disabled>
  ...
  <option value="svg:multiply-inherit-alpha">Multiply Inherit Alpha</option>
  <option value="svg:add">Addition</option>
  <option value="svg:add-inherit-alpha">Addition Inherit Alpha</option>
<!--  <option value="svg:subtract">Subtract</option> -->
  <option value="krita:erase">Erase</option>
</select>
`;
        ...
        // Clear the edit_tools array,
        edit_tools.length = 0;

        // Then add the buttons to the tools options,
...
//        camera.rotation.z += 0.05 * Math.PI / 180;
    ...
    function updatePoseMeshObject(mesh_ob) {
        if (mesh_ob !== undefined) {
            ...
            if (mesh_uid === current_selected_mesh_uid) {
                mesh_details = mesh_editor.getCurrentMeshDetails();
            }
            else {
                mesh_details = {
            ...
            updatePoseMesh(mesh_uid, mesh_details);
        }
    }
    ...
    function initializeTextureLayers() {
        m_three_cache.initializeTextureLayers(
                                ss, getRenderer(), composition_controller);
    }
...
//        console.log("EditorPanel INITIALIZE");
        ...
        composition_controller = CompositionController(m_three_cache);
        ...
        // Editor scratch (current_mode, )
        const editor_scratch = ss.get('editor_scratch');
        if (editor_scratch === undefined) {
            ss.set('editor_scratch', {
                current_view_mode: 'edit',
                current_tool_mode: 'mesh_edit_mode',
            });
        }
        ...
        model_compositor = ModelCompositor(m_three_cache);
        model_compositor.loadFrom(ss);
    ...
    function notify(event_type, ...args) {
        ...
        else if (event_type === 'create_lattice_action') {
            doCreateLatticeAction();
        }
        else {
