    ...
    function handleViewPortLeftMouseDown(evt) {
        ...
        if (current_selected_mesh_uid !== undefined) {
            ...
            if (vertex_uids.length > 0) {
                ...
                const weighted_selection = mesh_editor.createWeightedSelection(64);
                active_mesh_editing = {
                ...
                console.log(active_mesh_editing);
...
//        console.log(evt);
    ...
    function moveVertexesAction(now_r) {
...
//        console.log("PENDING: Move %o vertexes", weighted_selection);

        mesh_editor.moveWeightedVertexes(weighted_selection, dx, dy);

//        mesh_editor.moveSingleVertex(
//                    active_mesh_editing.selected_vertex_uid, now_r.x, now_r.y);
        active_mesh_editing.mutation = true;
        update_mesh_editor = true;
        updatePoseMesh( current_selected_mesh_uid,
                        mesh_editor.getCurrentMeshDetails() );
    ...
    function handleInteractiveActionUpdate(action_uid, value) {
        // Update the pose mesh so that it reflects the current state,
        composition_controller.updatePoseFromSingleAction(
                                                    ss, action_uid, value);
    }
