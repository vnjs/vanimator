...
//        camera.rotation.z += 0.05 * Math.PI / 180;
    ...
    function setLayerPoseGeometry(layer_uid, vertex_arr, face_indexes) {
        ...
        const layer_x = layer_ob.get('x');
        const layer_y = layer_ob.get('y');
        const layer_width = layer_ob.get('width');
        const layer_height = layer_ob.get('height');
    ...
    function updatePoseMesh(mesh_uid, mesh_details) {
        ...
        if (layers_set !== undefined) {
            layers_set.forEach( (layer_uid) => {
                setLayerPoseGeometry(layer_uid,
