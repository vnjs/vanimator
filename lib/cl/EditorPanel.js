...
//        camera.needsUpdate = true;
    ...
    function refreshLayerBlendMode(uid) {
        const layer_ob = getLayerFromUid(uid);
        if (layer_ob !== undefined) {
            const layer_blend = layer_ob.get('blend');
            const layer_cache_ob = threejs_cache[uid];
    ...
    function refreshLayerOpacity(uid) {
        const layer_ob = getLayerFromUid(uid);
        if (layer_ob !== undefined) {
            const layer_opacity = layer_ob.get('opacity');
            const layer_cache_ob = threejs_cache[uid];
            if (layer_cache_ob !== undefined) {
                const pose_material = layer_cache_ob.pose_material;
                const present_material = layer_cache_ob.present_material;
                pose_material.opacity = layer_opacity;
                present_material.opacity = layer_opacity;
                pose_material.needsUpdate = true;
                present_material.needsUpdate = true;
            }
        }
...
//            }
    ...
    function handleVertexDrag(evt) {
        ...
        if (active_mesh_editing !== undefined) {
            ...
            updatePoseMesh(current_selected_layer_uid,
                        mesh_details.vertices, mesh_details.face_indexes);
        }
...
//        console.log(evt);
    ...
    function handleBlendModeChange(evt) {
        const layer_ob = getLayerFromUid(current_selected_layer_uid);
        if (layer_ob !== undefined) {
            const new_layer_blend_mode = layer_blend_mode_select_el.value;
            layer_ob.set('blend', new_layer_blend_mode);
            refreshLayerBlendMode(current_selected_layer_uid);
            // Refresh the scene,
            refreshScene();
            // Checkpoint for UNDO
            replay_history.checkpoint();
...
//        console.log("= ", layer_blend_mode_select_el.value);
    ...
    function handleMeshModeChange(evt) {
        console.log("MESH MODE CHANGE: ", evt);
    }
    ...
    function handleLayerOpacityChange(evt) {
        ...
        if (layer_ob !== undefined) {
            ...
            refreshLayerOpacity(current_selected_layer_uid);
            // Refresh the scene,
            refreshScene();
            // Checkpoint for UNDO
            replay_history.checkpoint();
        }
//        console.log("Set opacity of layer %s to %s",
//                                current_selected_layer_uid, opacity_value);
    }
...
`;
        ...
        const project_tree_actions = document.createElement('div');
        project_tree_actions.className = 'noselect project-tree-actions';
        project_tree_actions.appendChild(blend_pulldown);
        project_tree_actions.appendChild(opacity_slider.domElement);

        // Disable the opacity slider,
        opacity_slider.disable();

        const project_tree_content = document.createElement('div');
        project_tree_content.className = 'noselect project-tree-content';
        project_tree_content.appendChild(layers_tree_panel.domElement);

        ui_left1_content.appendChild(project_tree_actions);
        ui_left1_content.appendChild(project_tree_content);
...
<select id="edit_mesh_mode" disabled>
  <option value="edit">Present Mode</option>
...
`;
    ...
    function updatePoseMesh(layer_uid, vertices, face_indexes) {
        const meshes = ss.getArray('meshes');
        meshes.forEach((layer) => {
            if (layer.get('uid') === layer_uid) {
...
//                        color: 0x0ff4020,
                ...
                const vertices = layer.get('me_vertices');
                const face_indexes = layer.get('me_face_indexes');
                ...
                refreshLayerBlendMode(layer.get('uid'));
